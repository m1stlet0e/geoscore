...
const SEVERITY: Record<string, { ring: string; dot: string; text: string; label: string; icon: React.ReactNode }> = {
  high: { ring: 'border-l-rose-500/60', dot: 'bg-rose-400', text: 'text-rose-600', label: '高', icon: <AlertTriangle className="h-4 w-4" /> },
  medium: { ring: 'border-l-amber-500/60', dot: 'bg-amber-400', text: 'text-amber-600', label: '中', icon: <Bell className="h-4 w-4" /> },
  low: { ring: 'border-l-slate-500/60', dot: 'bg-neutral-500', text: 'text-neutral-700', label: '低', icon: <Sparkles className="h-4 w-4" /> },
};
    ...
    <div className="space-y-6">
          ...
          return (
            ...
            >
              ...
              {t.id === 'unread' && unreadCount > 0 && (
                <span className="ml-1.5 inline-flex h-4 min-w-4 items-center justify-center rounded-full bg-rose-500/20 px-1 text-xs text-rose-600">
                  {unreadCount}
                </span>
              )}
            </Link>
      ...
      {alerts.length === 0 ? (
        <div className="rounded-2xl border border-neutral-200 bg-neutral-100 p-12 text-center">
          ...
          <p className="mt-2 text-sm text-neutral-500">所有指标都在正常范围</p>
        </div>
      ) : (
          ...
          {alerts.map((a) => {
            ...
            return (
              <div
                key={a.id}
                className={cn(
                  'flex items-start gap-3 rounded-2xl border border-neutral-200 border-l-4 bg-neutral-100 p-4 transition',
                  sev.ring,
              ...
              >
                ...
                <div className="flex-1 space-y-1">
                  <div className="flex flex-wrap items-center gap-2">
                    ...
                    {a.brand && (
                      <span className="rounded-md bg-neutral-200/40 px-2 py-0.5 text-xs text-neutral-500">
                        {a.brand.name}
                      </span>
                    )}
                    {!a.isRead && <span className="h-2 w-2 rounded-full bg-indigo-400" />}
                  </div>
                  <h3 className="text-sm font-medium text-neutral-800">{a.title}</h3>
