...
const STATUS_STYLES: Record<AnalysisStatus, { bg: string; text: string; label: string }> = {
  pending: { bg: 'bg-slate-500/15', text: 'text-slate-300', label: '待处理' },
...
const GAP_TYPE_COLORS: Record<string, { bg: string; text: string; border: string }> = {
  ...
  schema: { bg: 'bg-indigo-500/15', text: 'text-indigo-300', border: 'border-indigo-500/30' },
  media: { bg: 'bg-pink-500/15', text: 'text-pink-300', border: 'border-pink-500/30' },
  github: { bg: 'bg-slate-500/15', text: 'text-slate-300', border: 'border-slate-500/30' },
...
export default function GapsPage() {
  const [brandId] = useState<string>('default-brand');
    ...
    return (
      <div className="space-y-8">
        ...
        <div className="flex flex-col items-center justify-center py-24 text-center">
          ...
          <h3 className="text-lg font-semibold text-slate-200">暂无差距分析数据</h3>
  ...
  function NewAnalysisModal() {
    return (
      <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm">
        <div className="w-full max-w-lg rounded-2xl border border-slate-800/70 bg-slate-900 p-6 shadow-2xl">
          <div className="flex items-center justify-between mb-5">
            <h3 className="text-lg font-semibold text-slate-100">新建差距分析</h3>
            <button
          ...
          <div className="space-y-4">
            <div>
              <label className="mb-1.5 block text-sm font-medium text-slate-300">AI 平台</label>
            ...
            <div>
              <label className="mb-1.5 block text-sm font-medium text-slate-300">Prompt 文本</label>
              <textarea
                value={newPromptText}
                onChange={(e) => setNewPromptText(e.target.value)}
                placeholder="输入要分析的 prompt，例如：推荐一款项目管理工具"
                rows={4}
                className="w-full rounded-xl border border-slate-700/60 bg-slate-800/40 px-4 py-3 text-sm text-slate-200 placeholder-slate-500 outline-none transition focus:border-indigo-500/50 focus:ring-1 focus:ring-indigo-500/30 resize-none"
              />
            </div>
            ...
            <div className="flex justify-end gap-3 pt-2">
              <button
                onClick={() => { setShowModal(false); setSubmitError(null); }}
                className="rounded-lg border border-slate-700/60 bg-slate-800/40 px-4 py-2 text-sm font-medium text-slate-300 hover:bg-slate-700/40 transition"
              >
                取消
              </button>
              <button
                ...
                className="inline-flex items-center gap-2 rounded-lg bg-gradient-to-r from-indigo-500 to-violet-500 px-4 py-2 text-sm font-semibold text-white shadow-lg shadow-indigo-500/20 transition hover:from-indigo-400 hover:to-violet-400 disabled:opacity-50 disabled:cursor-not-allowed"
              >
                {submitting ? (
                  <>
      ...
      <div className="flex flex-wrap items-center gap-3">
        <div className="relative">
          <select
            ...
            className="appearance-none rounded-lg border border-slate-700/60 bg-slate-800/40 px-4 py-2 pr-8 text-sm text-slate-300 outline-none transition focus:border-indigo-500/50"
          >
      ...
      {activeTab === 'list' && (
        <div className="space-y-4">
          {loadingAnalyses ? (
            ...
            <div className="rounded-2xl border border-slate-800/70 bg-gradient-to-b from-slate-900/80 to-slate-950/60 p-12 text-center">
              <Search className="mx-auto mb-3 h-8 w-8 text-slate-600" />
              <h3 className="text-base font-medium text-slate-300">暂无分析记录</h3>
              <p className="mt-1.5 text-sm text-slate-500">点击「新建分析」开始第一次 GEO 差距分析</p>
            </div>
          ) : (
            analyses.map((a) => {
              const plat = PLATFORM_COLORS[a.platform] ?? { bg: 'bg-slate-500/15', text: 'text-slate-300', border: 'border-slate-500/30', label: a.platform };
              const status = STATUS_STYLES[a.status] ?? STATUS_STYLES.pending;
              return (
                <div
                  key={a.id}
                  className="rounded-2xl border border-slate-800/70 bg-gradient-to-b from-slate-900/80 to-slate-950/60 p-5 transition hover:border-indigo-500/30"
                >
                  <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
                    <div className="flex-1 space-y-3">
                      ...
                      <div className="flex flex-wrap items-center gap-5 text-xs">
                        ...
                        <div className="flex items-center gap-1.5">
                          ...
                          <span className="font-mono text-slate-300">{a.benchmarkScore.toFixed(0)}</span>
                        </div>
                        <div className="flex items-center gap-1.5">
                          <span className="text-slate-500">差距</span>
                    ...
                    <button
                      ...
                      className="inline-flex shrink-0 items-center gap-1.5 rounded-lg bg-indigo-500/20 px-3.5 py-2 text-sm font-medium text-indigo-200 ring-1 ring-indigo-500/30 transition hover:bg-indigo-500/30 disabled:opacity-50"
                    >
                      {loadingDetail ? (
      ...
      {activeTab === 'detail' && (
        <div className="space-y-6">
          {!selectedAnalysis ? (
            <div className="rounded-2xl border border-slate-800/70 bg-gradient-to-b from-slate-900/80 to-slate-950/60 p-12 text-center">
              <Search className="mx-auto mb-3 h-8 w-8 text-slate-600" />
              <h3 className="text-base font-medium text-slate-300">请选择一个分析</h3>
              <p className="mt-1.5 text-sm text-slate-500">在「分析列表」中点击「查看详情」查看差距详情</p>
              <button
                onClick={() => setActiveTab('list')}
                className="mt-4 inline-flex items-center gap-1.5 rounded-lg bg-indigo-500/20 px-3.5 py-2 text-sm font-medium text-indigo-200 ring-1 ring-indigo-500/30 hover:bg-indigo-500/30 transition"
              >
                返回列表
              </button>
            </div>
            ...
            <>
              ...
              <div className="rounded-2xl border border-slate-800/70 bg-gradient-to-b from-slate-900/80 to-slate-950/60 p-6">
                <div className="mb-4 flex items-center justify-between">
                  <div>
                    <h2 className="text-base font-semibold text-slate-100">评分对比</h2>
                  ...
                  <button
                    ...
                    className="rounded-lg border border-slate-700/60 bg-slate-800/40 px-3 py-1.5 text-xs font-medium text-slate-400 hover:text-slate-200 transition"
                  >
                    返回列表
                  </button>
                </div>
                ...
                <div className="grid gap-6 sm:grid-cols-2">
                  ...
                  <div>
                    <div className="mb-2 flex items-center justify-between text-xs">
                      <span className="text-slate-400">品牌评分</span>
                      <span className={`font-mono font-semibold ${getScoreColor(selectedAnalysis.score)}`}>
                  ...
                  <div>
                    <div className="mb-2 flex items-center justify-between text-xs">
                      <span className="text-slate-400">基准评分</span>
              ...
              <div className="rounded-2xl border border-slate-800/70 bg-gradient-to-b from-slate-900/80 to-slate-950/60 p-5">
                <div className="mb-4">
                  <h2 className="text-base font-semibold text-slate-100">差距项目</h2>
                  <p className="mt-0.5 text-xs text-slate-400">按优先级排序</p>
                </div>
                      ...
                      <tbody className="divide-y divide-slate-800/40">
                        {[...(selectedAnalysis.gapItems ?? [])]
                          ...
                          .map((item) => {
                            ...
                            const typeStyle = GAP_TYPE_COLORS[item.type] ?? { bg: 'bg-slate-500/15', text: 'text-slate-300', border: 'border-slate-500/30' };
                              ...
                              <tr key={item.id} className="transition hover:bg-slate-900/30">
                                ...
                                <td className="px-3 py-3 text-center">
                                  <select
                                    ...
                                    className="rounded-md border border-slate-700/60 bg-slate-800/60 px-2 py-1 text-[11px] text-slate-300 outline-none"
                                  >
      ...
      {activeTab === 'tracking' && (
        <div className="space-y-6">
          ...
          <section className="grid gap-4 lg:grid-cols-2">
            ...
            <div className="rounded-2xl border border-slate-800/70 bg-gradient-to-b from-slate-900/80 to-slate-950/60 p-5">
              <div className="mb-3">
                <h2 className="text-base font-semibold text-slate-100">差距类型分布</h2>
                <p className="mt-0.5 text-xs text-slate-400">按差距类型统计</p>
              </div>
            ...
            <div className="rounded-2xl border border-slate-800/70 bg-gradient-to-b from-slate-900/80 to-slate-950/60 p-5">
              <div className="mb-3">
                <h2 className="text-base font-semibold text-slate-100">影响分布</h2>
                <p className="mt-0.5 text-xs text-slate-400">各类型差距的累计影响</p>
              </div>
          ...
          {!selectedAnalysis && analyses.length === 0 && (
            <div className="rounded-2xl border border-slate-800/70 bg-gradient-to-b from-slate-900/80 to-slate-950/60 p-12 text-center">
              ...
              <h3 className="text-base font-medium text-slate-300">暂无改进跟踪数据</h3>
